...
curve_model <- function(bonds_list, market_data, calc_date, model = "NS", init_guess = NULL, adj_dur = TRUE, 
    adj_vol = FALSE, max_vol = NULL) {
    ...
    if (is.null(init_guess)) 
        init_guess <- rep(0, num_model)
    
    # calculate paramteres 4 times: 1. with the initial guess by user 2. with the default initial guess 3.
    # with 2 random initial guesses (uniform) and take the best solution
    ...
    if (adj_dur == TRUE) {
        market_data <- cbind(market_data, duration = apply(market_data, 1, function(x) calc_bond_name(bonds_list, 
            x["name"], calc_date, as.numeric(x["market_price"]))$duration))
    }
    
    # Create a data frame that contains all bonds from the list and their market_data vector of the names
    ...
    cost_func <- function(init_guess) {
        ...
        bonds_model_prices <- vapply(bonds_list, function(x) price_bond_model(x, calc_date, model = model, 
            model_params = init_guess), numeric(1))
